...
# --- 数据读取 ---
df = pd.read_excel("问题3/附件/附件1.xlsx", header=None, skiprows=1)
...
pd.DataFrame({"波长 (μm)": wavelength, "实验反射率": R_exp, "拟合反射率": R_fit}).to_excel(
    os.path.join(output_dir, "附件1拟合曲线数据.xlsx"), index=False
)
...
plt.savefig(os.path.join(output_dir, "附件1拟合结果.png"), dpi=300)
plt.show()
...
plt.legend(); plt.grid(alpha=0.3)
plt.savefig(os.path.join(output_dir, "附件1_干涉条纹标记.png"), dpi=300)
plt.show()

# --- 导出条纹数据 ---
pd.DataFrame({"条纹波长_μm": lambda_peaks, "条纹间隔Δλ_μm": np.append(delta_lambda, np.nan)}).to_excel(
    os.path.join(output_dir, "附件1_条纹间隔数据.xlsx"), index=False
)
# -------- 附加功能：厚度可靠性分析 (快速蒙特卡洛) --------
n_trials = 200
d_values = []
...
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "附件1_厚度分布_MC.png"), dpi=300)
plt.show()
