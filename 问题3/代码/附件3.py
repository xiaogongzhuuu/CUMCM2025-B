...
# --- 数据读取 ---
df = pd.read_excel("问题3/附件/附件3.xlsx", header=None, skiprows=1)
...
plt.savefig(os.path.join(output_dir, "附件3拟合结果.png"), dpi=300)
plt.show()
...
plt.legend(); plt.grid(alpha=0.3)
plt.savefig(os.path.join(output_dir, "附件3_干涉条纹标记.png"), dpi=300)
plt.show()

# --- 导出条纹数据 (基于滤波) ---
pd.DataFrame({"条纹波长_μm": lambda_peaks, "条纹间隔Δλ_μm": np.append(delta_lambda, np.nan)}).to_excel(
    os.path.join(output_dir, "附件3_条纹间隔数据.xlsx"), index=False
)
# -------- 附加功能：厚度可靠性分析 (快速蒙特卡洛) --------
n_trials = 200
d_values = []
...
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "附件3_厚度分布_MC.png"), dpi=300)
plt.show()
...
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "附件3_滤波后反射率.png"), dpi=300)
plt.show()