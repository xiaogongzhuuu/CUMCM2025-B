...
# --- 数据读取 ---
df = pd.read_excel("问题3/附件/附件2.xlsx", header=None, skiprows=1)
...
pd.DataFrame({"波长 (μm)": wavelength, "实验反射率": R_exp, "拟合反射率": R_fit}).to_excel(
    os.path.join(output_dir, "附件2拟合曲线数据.xlsx"), index=False
)
...
plt.savefig(os.path.join(output_dir, "附件2拟合结果.png"), dpi=300)
plt.show()
...
plt.legend(); plt.grid(alpha=0.3)
plt.savefig(os.path.join(output_dir, "附件2_干涉条纹标记.png"), dpi=300)
plt.show()
