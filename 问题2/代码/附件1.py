...
# 读取数据
data = pd.read_excel('问题2/附件/附件1.xlsx', header=None)
...
result.to_excel(os.path.join(output_dir, '波长_折射率_附件1.xlsx'), index=False)
...
with open(os.path.join(output_dir, '分析报告_附件1.txt'), 'w', encoding='utf-8') as f:
    f.write(report)
...
df_stripes.to_excel(os.path.join(output_dir, "条纹间隔_附件1.xlsx"), index=False)

with open(os.path.join(output_dir, "厚度计算结果_附件1.txt"), "w", encoding="utf-8") as f:
