...
pd.DataFrame({"波长_μm": wavelength, "折射率": n_cauchy(wavelength)}).to_excel(
    os.path.join(output_dir, "波长_折射率_附件1.xlsx"), index=False
)
pd.DataFrame({"条纹波长_μm": lambda_peaks, "条纹间隔Δλ_μm": np.append(delta_lambda, np.nan)}).to_excel(
    os.path.join(output_dir, "条纹间隔_附件1.xlsx"), index=False
)
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'SiC_10度拟合.png'), dpi=300)
...
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "滤波后反射率.png"), dpi=300)
plt.show()
